...
class YmlKeys 
   def initialize(f = nil , a = nil, l = 'fr')
      ...
      @path = "bo.facile/app/views/#{@folder}/" 
   end   


   def self.ignore(t = nil)
      t = self.left[0] if t.nil?
      File.open("yml_to_ignore.yml", "a+") do |f|         
         f.puts("    #{t}: \"#{t}\"" )          
      end             
   ...
   def self.next
      ...
      to_ignore = YAML.load_file('yml_to_ignore.yml')
      folders.each do |folder|
         if File.file?(File.join("bo.facile/app/views/#{folder}", 'locale.fr.yml'))
         else
            r = to_ignore['ignore'].key(folder)
               if(r.nil?)
                  results << folder
               else
